...
export const getLessonsByLearningObjectiveIdHandler = async (req: Request<{ learningObjectiveId: string }>, res: Response) => {
  try {
    const lessons = await lessonService.getLessonsByLearningObjectiveId(req.params.learningObjectiveId);
    res.status(200).json(createSuccessResponse(lessons, 'Lessons retrieved successfully'));
  } catch (error: any) {
    res.status(500).json(createErrorResponse('SERVER_ERROR', error.message));
  }
